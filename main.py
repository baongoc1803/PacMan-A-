...
class MovingEntity(Entity):
    ...
    def draw(self):
        screen.blit(self.sprite.subsurface(self.direction * self.imgPerCycle * self.size +
                    self.size * int(self.subImg), 0, self.size, self.size), (self.xPos, self.yPos))
...
class PacGum(StaticEntity):
    ...
    def draw(self):
        pygame.draw.rect(screen, pygame.Color(255, 183, 174),
                         (self.xPos + 8, self.yPos + 8, self.size, self.size))
...
class SuperPacGum(StaticEntity):
    ...
    def draw(self):
        if self.flicker < 30:
            pygame.draw.ellipse(screen, pygame.Color(
                255, 183, 174), (self.xPos, self.yPos, self.size, self.size))
...
class Ghost(MovingEntity):
    def __init__(self, xPos, yPos, sprite, time):
        ...
        self.ghost_frightened_1 = pygame.image.load(
            'res/img/ghost_frightened.png')
        self.ghost_frightened_2 = pygame.image.load(
            'res/img/ghost_frightened_2.png')
...
class InkyGhost(Ghost):
    ...
    def getChasePos(self):
        distance2Cell = getPointDistanceDirection(
            pacman.xPos, pacman.yPos, 32, dic[pacman.getDir()])
        distanceToBlinky = getDistance(
            distance2Cell[0], distance2Cell[1], self.otherGhost.xPos, self.otherGhost.yPos)
        directionToBlinky = getDirection(
            self.otherGhost.xPos, self.otherGhost.yPos, distance2Cell[0], distance2Cell[1])
...
def checkFood():
    ...
    r = pygame.Rect((pacman.xPos + pacman.size / 2 - 8,
                    pacman.yPos + pacman.size / 2 - 8), (16, 16))
    for i in entities:
